...
def ajax_dublincore(request, coi=''):  # 存Dublincore table make_poi
    ...
    if request.method == 'POST':
        ...
        if push_media_type==None or push_media_type=="":
            push_media_type = "0"
        if int(push_media_type)!=int(media) and int(push_media_type)!=0:
            del_media = models.Mpeg.objects.filter(~Q(format=int(push_media_type)),foreignkey=poi_id)
            print("1.",del_media)
            del_media.delete()
        else:
            if push_media_type!="1" and int(push_media_type)!=0:
...
def get_verification_coi(request, coi): # sdc驗證者頁面
    ...
    if content == 'poi':
        if user_role != 'admin' and user_role != 'identifier':
            ...
            all_xoi = all_xoi.filter(identifier=role, language=language)
        values_list = list(all_xoi.values('poi_id', 'poi_title', 'identifier', 'rights'))
        data = {"all_poi": values_list}
        return JsonResponse(data)
    elif content == 'loi':
        if user_role != 'admin' and user_role != 'identifier':
